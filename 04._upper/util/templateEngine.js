...
//parser script som tekst
import escape from "escape-html";




//false values; null, undefined, 

//SPA = is no serversite rendering - 

function renderPage(page, config={}) { 
    const navbar = fs.readFileSync("./public/components/navbar/navbar.html").toString()
    .replace("$TAB_TITLE", config.tapTitle || "upper")
...
async function renderJokePage(){
    ...
    if(joke.joke){
        ...
        const jokeHtmlContent = 
            `<h3>${escape(joke.setup)} : ${escape(joke.delivery)}</h3>`;
